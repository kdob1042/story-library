...
| 漫画構成 | 同じチャットAIが原稿理解→構成→レビュー | `manga/<episodeId>/episode.json` と `pages/<pageId>.json`。任意の短い決定メモ |
...
漫画アプリには原文入りのページネームと固定人物ID別の参照画像を渡す。原稿の別途取込みは不要。前半・後半はページの選択範囲で、必要なページから作れる。最新原稿や画像とのhash一致を要求しない。書出しと変更ページだけの検証はManga Directorの[handoff](../skills/manga-director/handoff.md)に従う。
