...
3. 原稿とネームのscene ID、対象範囲、原文hash、掲載文字、参照画像・設定版を、repo内にvendorした同一name-plan/v2 contractで検査する。hashはコードで計算し、文字offsetも既存SourceRef変換を使う。人がJSONを修正しないと通らない状態を完成にしない。
